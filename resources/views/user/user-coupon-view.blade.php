    ...
    <div id="coupon-card-pc">
        <div class="coupon-card-header justify-content-center notice-card-header mt-3 position-relative" style="display: flex;">
            <ul class="nav nav-underline mt-3" style="font-size: 16px">
                <li class="nav-item">
                    <a class="nav-link active" id="link-coupon-purchase" data-set="purchase" href="#">{{ __('userpage.coupon_purchase') }}</a>
                </li>
                <li class="nav-item">
                    <a class="nav-link" id="link-coupon-history" data-set="history" href="#">{{ __('userpage.purchase_history') }}</a>
                </li>
            </ul>
        ...
        <div class="container form-group coupon-card-body-purchase" style="margin: auto;">
            <div class="row pb-3" id="coupon-card-layout" style="margin: 0 auto; display: flex; background-color: #DCE0ED;">
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $('.coupon-card-header .nav-link').click(function(){
                ...
                if(data_set === 'purchase'){
                    ...
                    $('#coupon-history-layout').css({'display':'flex'});
                    mpstart = 1;
                    getCouponHistory();
                }
            });
                    ...
                    if (data.msg === "ok") {
                        ...
                        for (let i = 0; i < lists.length; i++) {
                            ...
                            if (parseInt(list.status) === 1) {
                                status = '{{ __('userpage.in_use') }}';
                            } else if(parseInt(list.status) === 2) {
                                status = '{{ __('userpage.expiration') }}';
                            } else if(parseInt(list.status) === 3) {
                                status = '{{ __('userpage.used') }}';
                            }
        ...
        function showCouponCard(cards) {
            ...
            for (let i = 0; i < cards.length; i++) {
                ...
                tag += '</div>';
                tag += div_item;
                tag += '<span class="mb-2 md-bold-font">$'+given+' {{ __('userpage.offered') }}</span>';
                tag += '</div>';
                tag += div_item;
                ...
                tag += '</div>';
                tag += div_item;
                tag += '<span class="md-bold-font">'+coins+' {{ __('userpage.coin_unit') }}</span>';
                tag += '</div>';
                ...
                tag += '{{ __('userpage.coupon_purchase') }}';
                tag += '</div>';
                tag += '</div>';
                tag += '</div>';
                tag += '</div>';
            ...
            $('div[id^="purchase-coupon-btn_"]').click(function(){
                ...
                window.location.href = "/user.user-coupon-payment-view/card-num/"+card_num;
            });
        }
