            ...
            <div style="width: 60%">
                <div class="header mt-5">
                    <div class="d-flex align-items-center pl-3 pt-1 pb-1 pr-3 position-absolute" style="left:2rem; width: auto; height: 1.5rem; cursor: pointer;">
                        <a class="d-flex" href="{{ url('/user_mobile.user-coupon-view/user-page/coupon') }}">
                            <img src="{{URL::asset('assets/img/pngs/back_left.png')}}">
                            <span class="d-flex ml-2">{{ __('userpage.coupon') }}</span>
                        </a>
                    </div>
                    <h5 id="payment_label" style="text-align: center; color: #404450;">{{ __('userpage.coupon_purchase_payment') }}</h5>
                    ...
                    <div class="row mt-2 justify-content-center">
                        <div class="col-6">
                            <div class="nav nav-tabs nav-fill mt-3" id="nav-tab" role="tablist">
                                <a class="d-flex pay-item pay-link active" id="nav-usdt-tab" data-toggle="tab" href="#nav-usdt">{{ __('userpage.usdt_payment') }}</a>
                                <a class="d-flex pay-item pay-link" id="nav-point-tab" data-toggle="tab" href="#nav-point">{{ __('userpage.point_payment') }}</a>
                                <a class="d-flex pay-item pay-link" id="nav-binance-tab" data-toggle="tab" href="#nav-binance">{{ __('userpage.binance_pay') }}</a>
                            </div>
                        </div>
                        ...
                        <div class="tab-content d-flex pt-5 pb-4 justify-content-center" id="nav-tabContent">
                            <div class="tab-pane fade show active" id="nav-usdt" role="tabpanel">
                                <div class="mb-5" style="width: 80%; margin: 0 auto;">
                                    <div class="row mb-3">
                                        <div class="col-4" style="text-align: end;">{{ __('userpage.deposit_usdt_address') }}</div>
                                        <div class="col-8" id="input_usdt_address"></div>
                                    </div>
                                    <div class="row mb-3">
                                        <div class="col-4" style="text-align: end;">{{ __('userpage.deposit_network') }}</div>
                                        <div class="col-8" id="input_usdt_network">Tron(TRC20)</div>
                                    </div>
                                    <div class="row mb-3 align-items-center">
                                        <div class="col-4" style="text-align: end;">{{ __('userpage.TxID') }}</div>
                                        <div class="col-8">
                                            <input type="text" class="form-control form-control-user my-input" id="input_txid">
                                        </div>
                                    </div>
                                    <div class="row mb-3">
                                        <div class="col-12" style="font-size:0.85rem; text-align: center; color: #F14382">{{ __('userpage.usdt_warning') }}</div>
                                    </div>
                                    <div class="row mb-3 justify-content-center">
                                        <div id="btn_payment_usdt" class="btn btn-black btn-user btn-block w-50">{{ __('userpage.dispot_comp') }}</div>
                                    </div>
                                </div>
                                <div class="mb-1">
                                    <div class="row"><div class="col-12 mb-1">{{ __('userpage.usdt_desc1') }}</div></div>
                                    <div class="row"><div class="col-12 mb-1">{{ __('userpage.usdt_desc2') }}</div></div>
                                    <div class="row"><div class="col-12 mb-3">{{ __('userpage.usdt_desc3') }}</div></div>
                                    <div class="row" style="text-align: center;">
                                        <div class="col-12 justify-content-center">
                                            <img src="{{URL::asset('assets/img/pngs/binance_ex.png')}}" style="">
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="tab-pane fade" id="nav-point" role="tabpanel">
                                <div class="w-75" style="margin: 0 auto;">
                                    ...
                                    <div class="row mb-3 justify-content-center" style="font-size: 0.9rem;">{{ __('userpage.points_desc1') }}</div>
                                    <div class="row mb-3 points-content" style="margin: 0 auto; width: 95%; font-size: 0.9rem">
                                        <div class="col-12 mb-3">
                                            <div class="row">
                                                <div class="col-6" style="text-align: end;">{{ __('userpage.points_1') }}</div>
                                        ...
                                        <div class="col-12">
                                            <div class="row">
                                                <div class="col-6" style="text-align: end;">{{ __('userpage.points_2') }}</div>
                                    ...
                                    <div class="row justify-content-center">
                                        <div id="btn_payment_point" class="btn btn-black btn-user btn-block w-50">{{ __('userpage.payment') }}</div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="row w-75 justify-content-center pay-footer-content" style="margin: 0 auto;">
                        <div class="tab-content d-flex p-3 w-100">
                            <div class="w-100 mb-5" id="pay-footer-usdt" style="display: block;">
                                <ul>
                                    <li>{{ __('userpage.usdt_desc4') }}</li>
                                </ul>
                            </div>
                            <div class="w-100 mb-5" id="pay-footer-point" style="display: none;">
                                <ul>
                                    <li>{{ __('userpage.points_desc2') }}</li>
                                    <li>{{ __('userpage.points_desc3') }}</li>
                                    <li>{{ __('userpage.points_desc4') }}</li>
                                </ul>
                            </div>
...
@section('js')
    <script>
        ...
        function showSelectedCard(datas) {
            ...
            tag += '</div>';
            tag += div_item;
            tag += '<span class="mb-2 md-bold-font">$'+datas.amount_given+' {{ __('userpage.offered') }}</span>';
            tag += '</div>';
            tag += div_item;
            tag += '<span>{{ __('userpage.max_trade_amount') }}</span>';
            tag += '</div>';
            tag += div_item;
            tag += '<span class="mb-2 md-bold-font">$'+my_max_amount+'</span>';
            tag += '</div>';
            tag += div_item;
            tag += '<span>{{ __('userpage.trade_coins') }}</span>';
            tag += '</div>';
            tag += div_item;
            tag += '<span class="md-bold-font">'+datas.coin_count+' {{ __('userpage.coin_unit') }}</span>';
            tag += '</div>';
            tag += '<hr class="sidebar-divider">';
            tag += div_item;
            tag += '<span>{{ __('userpage.price') }}</span>';
            tag += '</div>';
            tag += div_item;
