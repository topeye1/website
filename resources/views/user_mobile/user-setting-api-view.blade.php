<div id="setting_api_add" style="display:none;">
    ...
    <div class="pl-3 pr-3 mt-3">
        <div>{{ __('userpage.secret_key') }}</div>
        <div>
            <div class="d-inline-block" style="width: 90%">
    ...
    <div class="mt-5 mb-3">
        <img src="{{URL::asset('assets/img/pngs/ex_apiKey.png')}}" style="display:block; width: 100%;">
    </div>
</div>
<div id="setting_api_list" style="display:none; height: 100%;">
    ...
    <div class="col-12 mt-3 mb-3 d-flex justify-content-end">
        <div id="btn_key_add" class="btn btn-confirm" style="width: 100px;">{{ __('userpage.add_api') }}</div>
    </div>
                ...
                if (data.msg === "ok") {
                    ...
                    if (data.lists.length > 0) {
                        ...
                        for (let i = 0; i < lists.length; i++) {
                            ...
                            tags += '        <div class="col-5 p-0">{{ __('userpage.number') }}</div>';
                            tags += '        <div class="col-7 p-0 d-flex justify-content-end">'+num+'</div>';
                            tags += '    </div>';
                            tags += '    <div class="row m-0 mb-2" style="border-bottom: 1px solid #e3e6f0;">';
                            tags += '        <div class="col-5 p-0">{{ __('userpage.category') }}</div>';
                            tags += '        <div class="col-7 p-0 d-flex justify-content-end">'+list.key_name+'</div>';
                            tags += '    </div>';
                            tags += '    <div class="row m-0 mb-2" style="border-bottom: 1px solid #e3e6f0;">';
                            tags += '        <div class="col-5 p-0">{{ __('userpage.input_date') }}</div>';
                            tags += '        <div class="col-7 p-0 d-flex justify-content-end">'+list.create_date+'</div>';
                            tags += '    </div>';
                            tags += '    <div class="row m-0">';
                            tags += '        <div class="col-5 p-0">{{ __('userpage.delete') }}</div>';
                            tags += '        <div class="col-7 p-0 d-flex justify-content-end">';
                            tags += '           <div id="btn-delete_'+kid+'" class="btn btn-user my-middle-btn btn-block" style="height: 22px; font-size: 0.75rem; width:50%;">' + '{{ __('userpage.delete') }}' + '</div>';
                            tags += '        </div>';
                            tags += '    </div>';
                            tags += '</div>';
                        }
