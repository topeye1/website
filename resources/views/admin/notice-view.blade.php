...
@section('content')
            ...
            <div class="card notice-card">
                <div class="card-header d-flex justify-content-center notice-card-header">
                    <ul class="nav nav-underline" style="font-size: 16px">
                        <li class="nav-item">
                            <a class="nav-link active" data-set="all" href="#">{{ __('userpage.notice_all') }}</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" data-set="agent" href="#">{{ __('userpage.notice_agent') }}</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" data-set="persion" href="#">{{ __('userpage.notice_personal') }}</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" data-set="details" href="#">{{ __('userpage.notice_details') }}</a>
                        </li>
                    </ul>
                </div>
                            ...
                            <div class="col-md-12">
                                <div class="row justify-content-center">
                                    <div class="col-md-4 pb-md-0 pb-sm-2">
                                        <input type="text" id="search_text" class="form-control" placeholder="{{ __('userpage.search') }}">
                                    </div>
                                ...
                                <div class="e-table">
                                    <div class="table-responsive table-lg">
                                        <table class="table table-bordered mb-0 text-center">
                                            <thead>
                                            <tr>
                                                <th style="width: 20%" >{{ __('userpage.user_number') }}</th>
                                                <th style="width: 40%" >{{ __('userpage.phone') }}</th>
                                                <th style="width: 40%" >{{ __('userpage.name') }}</th>
                                            </tr>
                                            </thead>
                ...
                <div class="card-body form-group notice-card-body-send" style="width: 50%; margin: 0 auto">
                    <div class="form-group">
                        <div class="row">
                            <div class="col-md-2">{{ __('userpage.notice_title') }}</div>
                            <div class="col-md-10">
                    ...
                    <div class="form-group">
                        <div class="row">
                            <div class="col-md-2">{{ __('userpage.notice_content') }}</div>
                            <div class="col-md-10">
                ...
                <div class="card-body form-group notice-card-body-details" style="display: none; border-top: 0; width: 80%; margin: 0 auto">
                    <div class="e-table form-group">
                        <div class="table-responsive table-lg">
                            <table class="table table-bordered mb-0 text-center">
                                <thead>
                                <tr>
                                    <th style="width: 15%">{{ __('userpage.notice_date') }}</th>
                                    <th style="width: 15%">{{ __('userpage.notice_target') }}</th>
                                    <th style="width: 25%">{{ __('userpage.notice_title') }}</th>
                                    <th style="width: 35%">{{ __('userpage.notice_content') }}</th>
                                    <th style="width: 10%">{{ __('userpage.notice_memo') }}</th>
                                </tr>
                                </thead>
                    ...
                    @include('layouts.page-navigation-func')
                </div>


                <div class="card-footer text-center card-notice-add-btn">
                    <div id="send_message_button" class="btn btn-success mt-1" style="width: 80px; margin-right: 30px;">{{ __('userpage.notice_send') }}</div>
                </div>
    ...
    @include('modals.notice-error-modal')
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $('#send_message_button').click(function () {
                ...
                if(title === ''){
                    $('#error_title').css({'display':'block','margin-top':'10px', 'color':'#d41b11'}).text('{{ __('userpage.err_notice_title') }}');
                    setTimeout(function () {
                ...
                if(content === ''){
                    $('#error_content').css({'display':'block','margin-top':'10px', 'color':'#d41b11'}).text('{{ __('userpage.err_notice_content') }}');
                    setTimeout(function () {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.msg === "ok") {
                            ...
                            $('#result_modal_title').css({'display':'block','margin-top':'10px', 'color':'#0BC334'}).text('{{ __('userpage.success_send_notice') }}');
                            setTimeout(function () {
                                $('#notice_title').val('');
                                $('#notice_content').val('');
                            ...
                            $('#ResultMessageModal').modal('show');
                            $('#result_modal_title').css({'display':'block','margin-top':'10px', 'color':'#d41b11'}).text('{{ __('userpage.failed_send_notice') }}');
                            setTimeout(function () {
                                $('#ResultMessageModal').modal('hide');
                            },1000);
                        }
                ...
                success: function (data) {
                    if (data.msg === "ok") {
                        ...
                        for (let i = 0; i < lists.length; i++) {
                            ...
                            let target = '';
                            if (type === "all") {
                                target = '{{ __('userpage.notice_all') }}';
                            } else if(type === "agent") {
                                target = '{{ __('userpage.notice_agent') }}';
                            } else {
